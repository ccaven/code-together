...
export async function initSandbox(frameContainer: HTMLDivElement, width: number) {
    ...
    function reload (code: string, regenerate: boolean = false) {
        ...
        sandbox.run("generateWorker(\"class Program { start () {  let canvas=null; let ctx=null; let raf=null; let postMessage=null; let Program = null; \" + function () { { " + code + " } }.toString().substr(12) + \" } }; (new Program()).start.bind({})();\", " + regenerate.toString() + ");");
    }
